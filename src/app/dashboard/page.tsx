...
import {
    getDiaryEntries, saveDiaryEntry,
    getMessages, sendMessage, markMessagesRead, getUnreadMessageCount,
    filterEntriesByPeriod, getPatientSelfAlert,
    type FilterPeriod,
} from "@/lib/mock-data";
...
export default function PatientDashboard() {
    ...
    useEffect(() => {
        ...
        setUser(currentUser);
        setEntries(getDiaryEntries(currentUser.id));
        setMessages(getMessages(currentUser.id));
        markMessagesRead(currentUser.id, "pasien");
        setSelfAlert(getPatientSelfAlert(currentUser.id));
    }, [router]);
    ...
    const handleSubmitDiary = () => {
        ...
        setLastResult(null);
        setTimeout(() => {
            const entry = saveDiaryEntry(user.id, text);
            setLastResult(entry);
    ...
    const handleSendMessage = () => {
        ...
        setSendingMsg(true);
        setTimeout(() => {
            sendMessage(user.id, "pasien", user.id, msgText);
    ...
    const unreadCount = user ? getUnreadMessageCount(user.id, "pasien") : 0;
                    ...
                    <Card className="shadow-card">
                        <CardContent className="flex flex-col p-0" style={{ height: "520px" }}>
                                ...
                                {messages.length === 0 ? (
                                    <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            Mulai percakapan dengan psikolog Anda di sini.
                                        </p>
                                    </div>
                                ) : (
                                    messages.map((msg) => {
                                        const isMe = msg.senderRole === "pasien";
                                        return (
                                            <div
                                                key={msg.id}
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <p className={`mt-1 text-[10px] ${isMe ? "text-white/60" : "text-muted-foreground"
                                                        ...
                                                        {isMe ? "Anda" : "Psikolog"} · {formatDate(msg.createdAt)}
                                                    </p>
                                                </div>
                                            </div>
                                        );
                            ...
                            <div className="border-t border-border p-4">
                                <div className="flex gap-2">
                                    <Textarea
                                        placeholder="Tulis pesan ke psikolog..."
                                        value={msgText}
