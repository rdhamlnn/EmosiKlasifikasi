...
import { getCurrentUser, getAllPatients } from "@/lib/auth";
import {
    getDiaryEntries, getNotes, saveNote,
    getMessages, sendMessage, markMessagesRead,
    filterEntriesByPeriod, getPatientStatus,
    markPatientSafe,
    type FilterPeriod,
} from "@/lib/mock-data";
import type { User, DiaryEntry, PsychologistNote, Message, AlertLevel } from "@/lib/types";
...
export default function PatientDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        if (!currentUser || currentUser.role !== "psikolog") {
            router.push("/login");
            return;
        }
        setUser(currentUser);
        ...
        const found = patients.find((p) => p.id === resolvedParams.id);
        if (found) {
            setPatient(found);
            setEntries(getDiaryEntries(found.id));
            setNotes(getNotes(found.id));
            setMessages(getMessages(found.id));
            markMessagesRead(found.id, "psikolog");
        }
    ...
    const handleSaveNote = () => {
        ...
        setSavingNote(true);
        setTimeout(() => {
            saveNote(user.id, patient.id, noteText);
            setNotes(getNotes(patient.id));
            setNoteText("");
            setSavingNote(false);
    ...
    const handleSendMessage = () => {
        ...
        setSendingMsg(true);
        setTimeout(() => {
            sendMessage(user.id, "psikolog", patient.id, msgText);
            setMessages(getMessages(patient.id));
            setMsgText("");
            setSendingMsg(false);
            toast.success("Pesan berhasil dikirim ke pasien!");
        }, 400);
    };

    const handleMarkSafe = () => {
        if (!user || !patient) return;
        markPatientSafe(user.id, patient.id);
        toast.success("Status pasien berhasil ditandai sebagai Aman.");
        // Force re-render by updating entries state
        setEntries([...getDiaryEntries(patient.id)]);
    };

    const handleGenerateReport = async () => {
        if (!patient) return;
        const { jsPDF } = await import("jspdf");
        const status = getPatientStatus(patient.id, patient.nama, patient.email);
        ...
        doc.text(`Psikolog: ${user?.nama || "-"}`, pageWidth / 2, 34, { align: "center" });
        ...
        doc.text("INFORMASI PASIEN", 14, y);
        y += 2;
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(29, 78, 216);
        doc.text("CATATAN PSIKOLOG", 14, y);
        y += 2;
        doc.setDrawColor(29, 78, 216);
        doc.line(14, y, pageWidth - 14, y);
        y += 8;
    ...
    return (
        <div className="py-8 md:py-12">
            <div className="container mx-auto px-4">
                {/* Back + Header */}
                <Link href="/psikolog" className="mb-4 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
                        ...
                        <div className="space-y-4">
                            ...
                            <Card className="shadow-card">
                                <CardContent className="space-y-3 p-5">
                                    <Textarea
                                        placeholder="Tulis catatan tentang pasien ini (hanya bisa dilihat Anda)..."
                                        value={noteText}
                                ...
                                <CardContent className="flex flex-col p-0" style={{ height: "400px" }}>
                                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                                        {messages.length === 0 ? (
                                            ...
                                            messages.map((msg) => {
                                                const isMe = msg.senderRole === "psikolog";
                                                return (
                                                    <div
                                                        key={msg.id}
                ...
                {activeTab === "laporan" && (
                    <div className="space-y-6">
                        <Card className="shadow-card">
                            <CardContent className="p-6">
                                <h3 className="mb-4 text-lg font-semibold text-foreground">Generate Laporan Pasien</h3>
                                <p className="mb-4 text-sm text-muted-foreground">
                                    Laporan akan berisi ringkasan emosi, distribusi emosi pada periode yang dipilih,
                                    entri diary terbaru, dan catatan psikolog.
                                </p>
                                    ...
                                    <CardContent className="p-4">
                                        ...
                                        <div className="space-y-3 text-sm">
                                            ...
                                            <div className="flex justify-between">
                                                <span className="text-muted-foreground">Catatan Psikolog</span>
