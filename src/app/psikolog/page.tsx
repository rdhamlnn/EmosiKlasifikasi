...
import { getPatientStatus } from "@/lib/mock-data";
import type { User, PatientStatus, AlertLevel } from "@/lib/types";
...
export default function PsychologistDashboard() {
    ...
    const safeCount = patientStatuses.filter((p) => p.alertLevel === "safe").length;
    const warningCount = patientStatuses.filter((p) => p.alertLevel === "warning").length;
    const criticalCount = patientStatuses.filter((p) => p.alertLevel === "critical").length;
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="mb-8">
                    ...
                    <p className="mt-1 text-muted-foreground">
                        Pantau kondisi emosional pasien Anda.
                    </p>
                </div>
                ...
                <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <Card className="shadow-card">
                        <CardContent className="flex items-center gap-4 p-5">
                            ...
                            <div>
                                <p className="text-2xl font-bold text-foreground">{patientStatuses.length}</p>
                ...
                <Card className="border-border/50 shadow-card">
                    <CardContent className="p-0">
                        <div className="border-b px-6 py-4">
                            <h2 className="text-lg font-semibold text-foreground">Daftar Pasien</h2>
                            <p className="text-sm text-muted-foreground">Klik untuk melihat detail emosi pasien</p>
                        </div>
                        {patientStatuses.length === 0 ? (
                            <div className="flex flex-col items-center gap-3 py-16 text-center">
                                <Users className="h-10 w-10 text-muted-foreground" />
                                <p className="font-semibold text-foreground">Belum ada pasien terdaftar</p>
                            ...
                            <div className="overflow-x-auto">
                                <Table>
                                    ...
                                    <TableBody>
                                        {patientStatuses.map((patient, i) => {
                                            const config = ALERT_CONFIG[patient.alertLevel];
                                            const AlertIcon = config.icon;
                                            return (
                                                <TableRow key={patient.patientId} className="group">
                                                    <TableCell className="text-muted-foreground">{i + 1}</TableCell>
                                                    <TableCell className="font-medium text-foreground">{patient.patientName}</TableCell>
                                                    <TableCell className="text-muted-foreground">{patient.patientEmail}</TableCell>
                                                    <TableCell>
                                                    ...
                                                    <TableCell className="text-center font-medium">{patient.totalEntries}</TableCell>
                                                    <TableCell className="text-center">
                                                        <span className={`font-semibold ${patient.negativePercentage > 60 ? "text-red-600" :
                                                                patient.negativePercentage >= 40 ? "text-amber-600" :
                                                                    "text-emerald-600"
                                                            }`}>
                                                            {patient.negativePercentage}%
                                                        </span>
                                                    </TableCell>
                                                    ...
                                                    <TableCell className="text-center">
                                                        <Link href={`/psikolog/pasien/${patient.patientId}`}>
